...
  // Si no se pasan argumentos devuelve 0. Si se pasa un argumento, simplemente devuélvelo
  ...
  var todosmultiplicados = 1;
  if (arguments.length === 0) return 0;
  if (arguments.length === 1) return arguments[0];
  for (i = 0; i < arguments.length; i++){
  todosmultiplicados = todosmultiplicados * arguments[i];
  }
  return todosmultiplicados;
}
...
function breakStatement(numero) {
  ...
  // Tu código:
  var NewArray = [];
  var SumandoDos = numero;

  for (var i = 0; i < 10; i++){
    SumandoDos = SumandoDos + 2;

    if (SumandoDos === i) break;
    NewArray.push(SumandoDos);
  }
